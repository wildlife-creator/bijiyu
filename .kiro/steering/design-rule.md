...
| CON-011 | 応募履歴一覧 |
| CLI-007 | 応募一覧（mypage・未対応のみ） |
| CLI-007B | 案件応募者一覧（CLI-002 導線・案件スコープ・全ステータス） |
| CLI-010 | 発注履歴一覧（mypage・決定以降） |
...
- 入力可能なフォーム要素（Input, Textarea, Select, date input）の背景色は `bg-background`（白）を使用する
- 薄いグレー背景（`bg-muted` 等）はフォーム要素に使用しない
...
- 実例: メッセージ入力欄・スカウトテンプレートフォーム・職種/資格/スキル検索コンボボックスの 5 箇所が 12〜14px で、ステージング確認「入力欄が拡大したまま戻らない」の原因になった
...
- 画面上の日付表示は `YYYY/MM/DD`（スラッシュ区切り）で統一する
- DB から取得した `YYYY-MM-DD` 形式の文字列は `formatDate()` ユーティリティ（`@/lib/utils/format-date`）を使って変換すること
- ハイフン区切り（`YYYY-MM-DD`）での日付表示は禁止
