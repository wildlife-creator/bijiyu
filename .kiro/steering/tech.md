...
| 通知系 | 求人へのお問い合わせ通知（発注者へ） | Server Action（`submitJobInquiryAction` 成功時。テンプレ `jobInquiryNotificationEmail`、宛先=宛先 client の `auth.users.email`） | job-inquiry |
...
各機能の spec-impl 完了後、その機能に関わるユーザーストーリーを網羅的に洗い出し、Playwright テストを作成する。
新しい機能の spec-impl 開始時の最初のステップとして `npm run test:e2e` を実行し、既存の全 Playwright テストが通ること（デグレードがないこと）を確認する。

**E2Eテスト作成の手順:**
1. まず「この機能で考えうるユーザーストーリー」を一覧化する（正常系 + エラー系）
2. 複数ロール（受注者・発注者・担当者）が関わる機能は、各ロールの視点でストーリーを書く
3. 各ストーリーについて、ユーザーの操作順にテストを書く（画面遷移→操作→結果確認の通しフロー）
4. 書き込み系操作（保存・アップロード・送信など）は必ずカバーする
5. seed.sql に必要なテストデータを事前に用意し、テストが確実に再現可能な状態にする
