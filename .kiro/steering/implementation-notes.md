...
### 19. 発注者詳細(CON-006)は職人詳細(CLI-006)のページデザインに統一 ✅ 対応済み（2026-07-11 決定）
- **決定事項**: 発注者詳細(`/clients/[id]`, CON-006)の**ページ全体のデザインを職人詳細(`/users/contractors/[id]`, CLI-006)に合わせる**。具体的には `bg-muted` 背景／90px アバター／見出し `text-[15px] font-bold tracking-wider`／基本情報は **`InfoRow`（薄紫ラベル帯 `bg-primary/[0.08]` ＋値行）のカード**／CTA は 2 ボタン横並び（アウトライン secondary ＋ primary、`rounded-[47px]`）を上下に配置。従来の `DetailRow`（枠なし下線行）は廃止し `InfoRow` に統一
- **カード外枠のグレー統一**: 詳細画面のカード外枠は、評価一覧画面(`/users/[id]/reviews`）の "テーブル" と同じ **`border border-border`（視認できるグレー線）** に統一する。職人詳細の従来の薄枠 `border-border/10`（ほぼ透明）も全カードで `border-border` に変更した（縁が見えないのが不自然だったため）。発注者詳細の新カードも `border-border`
- **`border-border/10` は詳細画面では使わない**: 今後、職人詳細・発注者詳細に新カードを足す際も `border border-border`（グレー）を使うこと。`border-border/10` は縁が実質不可視になり不採用（2026-07-11 のデザイン監査で確認）
- **職人詳細のレイアウトは変更しない**: 枠色のみグレー化。InfoRow の薄紫ラベル帯・段組はそのまま
- **旧メモの訂正**: 本項は当初「border/10（薄枠）を正とする」としていたが、ユーザーレビューで撤回。評価一覧画面と同じグレー枠に統一する方針へ変更した（Figma CON-006 のグレー枠とも整合する）