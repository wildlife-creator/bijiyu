...
| skill_tags | text[] (NOT NULL DEFAULT '{}') | 保有スキルタグ（例: 型枠設置、電気工、送配電線工）。COM-001 / COM-002 で表示・編集。`user_skills.trade_type`（職種大分類）とは役割が異なるので混同しないこと。詳細は後述 |

**`user_skills.trade_type` と `users.skill_tags` の使い分け:**

| フィールド | 用途 | 入力値の性質 | 上限 |
|---|---|---|---|
| `user_skills.trade_type` | 大分類（対応できる職種） | `TRADE_TYPES` から選択する固定値（大工・塗装・電気 等） | 1ユーザー最大3件 |
| `users.skill_tags` | 自由入力のスキルタグ（保有スキル） | ユーザーが任意に追加する詳細（型枠設置・外壁塗装 等） | 無制限（配列） |
...
※ 1ユーザーにつき最大3職種
※ 自由入力の「保有スキル」タグは `users.skill_tags`（text[]）に保存する。本テーブルはあくまで固定リストからの職種選択用
