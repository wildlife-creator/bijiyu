...
| operating_status | text | 稼働状況（'completed' / 'lost' 等） |
...
- 作成者向け: `owner_id = auth.uid() AND deleted_at IS NULL`
...
| user_reviews / client_reviews（評価） | そのまま残す | 公開情報として継続表示（退会ユーザー名は「退会済みユーザー」） |
...
| 操作 | 対象 | 条件 |
|------|------|------|
| SELECT | 全案件 | 認証済みユーザー全員（status = 'open' かつ deleted_at IS NULL のみ。draft/closed は作成者のみ） |
...
| UPDATE | — | 案件の作成者のみ（ステータス変更 = 発注可否の判断） |
...
| INSERT / UPDATE / DELETE | 案件の作成者のみ | `job.owner_id = auth.uid()` |

#### user_reviews（ユーザー評価 — 発注者→受注者）
| 操作 | 対象 | 条件 |
|------|------|------|
| SELECT | 全ユーザーが閲覧可（公開評価） | 全員可 |
| INSERT | 対象の応募の発注者のみ | `reviewer_id = auth.uid()` かつ応募ステータスが completed |
...
|------|------|------|
| SELECT | 全ユーザーが閲覧可（公開評価） | 全員可 |
| INSERT | 対象の応募の受注者のみ | `reviewer_id = auth.uid()` かつ応募ステータスが completed |
| UPDATE / DELETE | — | 不可（評価は変更・削除できない） |
