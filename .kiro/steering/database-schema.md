...
<!--
  ...
  - 無料ユーザーの場合: jobs.trade_types のいずれかが user_skills.trade_type の集合に含まれる（OR 一致）
    AND jobs.prefecture ∈ user_available_areas.prefecture
...
| テーブル | カラム | 用途 |
|---------|--------|------|
| jobs | (status, prefecture) | 案件検索（ステータス × エリア の複合 B-tree） |
...
**マスタテーブル参照（label 保存方式 / denormalization）にしたもの**（= 候補が業界で随時拡張する 3 種）:
...
詳細運用は次節「マスタテーブル参照（master-skills）」を参照。
