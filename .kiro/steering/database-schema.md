...
| address | text | 詳細住所(番地以下)。エリアは別テーブル `job_areas` で管理。DROP しないこと |
...
※ 旧 `jobs.prefecture` カラムは master-area Migration 4 で DROP 済。エリアは `job_areas` 別テーブルに移行(県跨ぎ・1 案件複数現場対応のため)。`jobs.address` は番地以下の詳細住所として保持
