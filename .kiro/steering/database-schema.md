...
### 発注者名に関するカラムの使い分け（必ず守ること）

発注者の「表示名」として解釈できるカラムが複数存在する。用途を誤ると「同じユーザーなのに画面によって名前が違う」現象が起きる。

| カラム | 型 | いつ設定されるか | 用途 |
|---|---|---|---|
| `users.company_name` | text | ユーザー登録時に屋号として入力 | **個人/小規模プラン**の発注者、または屋号を持つ受注者の表示名 |
| `organizations.name` | text | 法人プラン購入後の `/mypage/organization-setup`（CLI-021）で入力 | **active な法人プラン**のときに発注者名として表示 |
| `client_profiles.display_name` | text | プラン購入直後の自動生成（姓名結合） | 主に内部参照・代理アカウント等の識別用。UI 表示には使わない |
| `users.last_name` + `users.first_name` | text | 会員登録時 | 上記いずれも未設定時のフォールバック |

**表示解決のルール**（`src/lib/utils/display-name.ts` の `resolveParticipantName()`）:
1. `organizations.name`（active な法人プランのときのみ）
2. `users.company_name`
3. `${users.last_name}${users.first_name}`

**重要なポリシー**: データは保持し、表示は現在のプラン状態で切り替える
- ダウングレード/解約時も `organizations` レコードは削除しない（再アップグレードでの再利用のため）
- ただし表示は「`subscriptions.status='active'` AND `plan_type IN ('corporate','corporate_premium')`」のユーザーだけ `organizations.name` を使う
- この判定を束ねるヘルパー: `src/lib/utils/resolve-org-names.ts` の `getActiveCorporateOrgNames(admin, userIds)`

**RLS 上の注意**: `organizations` / `organization_members` は `is_same_org` RLS で保護されているため、他組織の情報は通常クライアントでは取得できない（nested join がサイレントに null になる）。発注者名の表示のような「他ユーザーの組織情報を表示する」場面では admin client（`createAdminClient()`）を使う。
