...
- [x] 7. マイリスト画面の実装（CON-007）
  ...
  - プルダウン切り替え（`<FavoriteTypeSelect>`）: 受注者は「案件」/「発注者」の2項目、発注者は「案件」/「発注者」/「見込みユーザー」の3項目。全N件は左、プルダウン＋並べ替えは右にまとめる
  - カード一覧は各一覧画面と**同一項目**: 案件タブは `JobListCard`（CON-002 共有）、発注者タブは CON-005 と同一（**募集エリア** `client_recruit_areas` を表示。個人所在地は出さない）、見込みユーザータブは CLI-005 と同一（高評価バッジ等まで）
  - **【residence/CON-007 改修 2026-06-05】案件表示中は締切並べ替え（`<FavoriteSortButton>`、`?sort=asc|desc`、既定 asc=締切が近い順）。B案=全件を締切順に並べてからページ分け（`jobs.recruit_end_date` 順）。発注者・見込みユーザーは並べ替えなし**
