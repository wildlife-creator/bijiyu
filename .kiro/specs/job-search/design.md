...
- CLI-006（職人詳細）では上記に加えて、「このユーザーは退会済みです。」の案内バナーを表示し、**空き日程セクションも非表示**にする（連絡が取れない相手の未来の空き日程を見せると誤解のもとになるため）
...
**フィルター項目**:
- CON-002（案件検索）: キーワード、エリア、希望日程、募集職種、経験年数、言語
...
**searchParams**:
- `q`: キーワード（jobs.title / jobs.description / client_profiles.display_name の3軸 OR 検索）
- `prefecture`: エリア
- `workPeriod`: 希望日程プリセット（1週間以内 / 2週間以内 / 1ヶ月以内 / 2ヶ月以内 / 3ヶ月以上先）→ work_start_date レンジに変換
- `tradeType`: 職種
- `experienceYears`: 経験年数（不問 / 1年以上 / 3年以上 / 5年以上 / 10年以上）
- `language`: 言語（LANGUAGES 定数の値1つ）→ jobs.language text[] への && 検索
...
- 「条件」セクション: 報酬、エリア、募集職種、募集人数、現場工期、募集期間、稼働時間、締め切り、経験年数、必須スキル、言語、持ち物
...
**レイアウト構造**:
- ページタイトル: 「マイリスト」
- フィルター行（1行）: **左に「全N件」、右に「種類プルダウン + 並べ替え矢印」をまとめる**（CON-007.png 準拠。位置はタブ間でブレないよう右寄せ固定）
  - 種類プルダウン（Select、`<FavoriteTypeSelect>`）:
    - 受注者: 「案件」/「発注者」
    - 発注者: 「案件」/「発注者」/「見込みユーザー」
  - 並べ替え矢印（`<FavoriteSortButton>`、`icon-sort.png` + 文字）: **案件表示中のみ表示**。締切が近い順 / 遠い順を切り替える（既定は近い順）
- 種類に応じたカード一覧（**各一覧画面と同じ共有部品・同じ項目に統一**）:
  - 案件タブ: `JobListCard`（CON-002 と共有部品）
  - 発注者タブ: CON-005 と**同一項目**（会社画像・会社名・会社住所・募集職種・募集エリア・求める働き方。エリアは `users.prefecture`（個人所在地）ではなく `client_recruit_areas`（募集エリア）を表示）
  - 見込みユーザータブ: CLI-005 と**同一項目**（高評価バッジ・氏名＋年齢・職種チップ・本人確認/CCUS バッジ・対応エリア・経験年数）
- 「マイリスト解除」ボタン + 「詳細をみる」ボタン
- 「もどる」ボタン

**searchParams**:
- `type`: `job`(default) / `client` / `user`（種類プルダウン）
- `sort`: `asc`(default = 締切が近い順) / `desc`(締切が遠い順)。**`type=job` のときのみ有効**
- `page`: ページ番号（種類ごとに独立）

**締切並べ替えの方式（B案・全件ソート）**: 案件は「お気に入り登録順」ではなく締切順で並べる。お気に入り案件の全 target_id を取得 → `jobs` を `recruit_end_date` 順で並べてから当該ページ分を取得（`range`）。複数ページにまたがっても正しく並ぶ。発注者・見込みユーザーは従来どおりお気に入り登録順でページ分け。
