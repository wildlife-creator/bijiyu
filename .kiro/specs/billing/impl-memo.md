...
- **CLI-021 実装時に success_url を全プラン CLI-021?setup=true に統一**（billing/actions.ts の `buildSuccessUrl` と BillingClient.tsx の 2 箇所）
  - 現行: 法人プランのみ `/mypage/organization-setup` 遷移、個人・小規模プランは `/mypage?checkout=success` 遷移
  - 移行後: **全プランで `CLI-021?setup=true` に統一**（法人は必須、非法人はスキップ可）
  - ⚠️ この TODO は **organization spec 側で実施**する作業として移管済み。詳細手順は `.kiro/specs/organization/requirements.md`「付録 A: 実装前提リファクタリング手順」**Step 4** を参照
  - 関連: `billing/tasks.md` Task 8.6（将来タスク）／Task 15.5（移行時に E2E 書き直し）
