...
- [x] 17. CLI-021 setup の法人プラン緩和（2026-06-10 仕様変更⑤・admin spec 招待フロー対応）— **2026-06-11 実装完了（同日のユーザー決定で内容を改訂）**
  - **確定仕様（2026-06-11 改訂）**: `?setup=true`（課金直後の初回設定）のみ募集職種・募集エリアを未入力可とする。**通常編集（edit モード）では従来どおり必須**（必須バッジ表示・未入力では保存不可）。社名は法人のみ必須（setup / edit 共通。姓名フォールバック不可のため）。個人・小規模の全体スキップは従来どおり
  - 当初案の「通常編集時も非必須 + マイページにナッジ表示」は**廃止**（ユーザー決定: 最終的に全発注者へ登録してもらう方針のため、通常編集の必須化で後からの入力を担保する。ナッジは不要）
  - 実装: `src/lib/validations/client-profile.ts` を法人/非法人 × setup/edit の 4 スキーマ構成に変更（`selectClientProfileSchema(planType, mode)`）。Server Action / CLI-021 フォームとも mode でスキーマと必須バッジを切替。法人 setup バナー文言は「プラン登録が完了しました。社名を入力してください。その他の項目は後からいつでも編集できます」（案C・ユーザー承認済）
  - テスト: Vitest（`src/__tests__/validations/client-profile.test.ts` 新規 + action テスト追加）/ `e2e/client-profile.spec.ts`（setup で空保存成功 → 通常編集で必須エラー → 再入力の通しテスト + 必須バッジ個数の期待値）
  - 経緯: `.kiro/specs/admin/requirements.md`「関連する仕様変更⑤（admin 範囲外 / CLI-021 緩和）」参照（2026-06-10 ユーザー承認 → 2026-06-11 実装時に目視確認を経て上記のとおり改訂）
  - _Requirements: 2.1_
