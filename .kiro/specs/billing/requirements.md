...
- フロー:
  ...
  3. 決済成功: success_url にリダイレクト
     - **全プラン共通**: CLI-021（発注者情報編集）に `?setup=true` 付きでリダイレクト。`setup=true` の場合、CLI-021 の画面上部にガイドバナーを表示する（個人・小規模: 「プラン登録が完了しました。発注者として利用する場合は社名または氏名を入力してください。受注者機能のみ利用する方はスキップ可（後からいつでも編集できます）」／法人: 「プラン登録が完了しました。社名を入力してください。その他の項目は後からいつでも編集できます」〔Task 17・2026-06-11 文言変更〕）。`client_profiles.display_name` はメッセージ画面で受注者に表示される名前に直結する
     - **法人プラン（corporate / corporate_premium）**: 社名のみ入力必須（**2026-06-10 仕様変更⑤・2026-06-11 改訂**: 募集職種・募集エリアは **setup 時のみ**未入力可に緩和。社名は法人で姓名フォールバックできないため必須維持。admin spec の招待フロー「会社名だけで即スタート」を成立させるための変更 — `.kiro/specs/admin/requirements.md`「関連する仕様変更⑤」参照）。「スキップ」ボタンは非表示のまま（社名未入力での全体スキップは不可）。保存完了後に CON-001 へ遷移。**通常編集（edit モード）では募集職種・募集エリアは従来どおり必須**（必須バッジ表示・未入力では保存不可）— 後からの入力はこの必須化で担保するため、マイページ等のナッジは設けない（2026-06-11 ユーザー決定で当初案のナッジを廃止。実装は Task 17）
...
- **プラン購入後の発注者情報入力（全プラン共通）**: success_url を全プランで CLI-021（`?setup=true`）に統一。背景は「`client_profiles.display_name` がメッセージ・案件カード・スカウト等で受注者に表示される唯一の名前源」であり、課金した瞬間に発注者名を設定する導線を持つべきため
  - **法人プラン**: 社名のみ入力必須（募集職種・募集エリアは **setup 時のみ**未入力可。通常編集では従来どおり必須。**2026-06-10 仕様変更⑤・2026-06-11 改訂** — 経緯は admin spec「関連する仕様変更⑤」、実装は Task 17 参照）。社名未入力での全体スキップは不可
