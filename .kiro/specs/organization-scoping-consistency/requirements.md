...
- お気に入り「見込みユーザー」（発注者がお気に入りした職人）の会社共有化。favorites は個人設計（`user_id` 単位・組織ID無し）のため、整合バグではなく新機能扱い。UX 判断（誰のお気に入りか／担当者が外せるか等）が要るため**別機能として後日**（2026-06-03 ユーザー決定: B）。受注者側のお気に入り（案件・発注者）は個人のまま据え置き
