...
> **【2026-06-10 更新（admin spec）】** 職場紹介動画（ADM-010B）の投稿入口は ADM-004（発注者アカウント詳細）へ移設が決定。ADM-009 のボタンは「受注者PR動画を投稿する」のみとなる（上記の video_workplace ボタン記述は実装当時の仕様。導線の撤去・E2E 書き換えは admin spec で実装）。
