...
4. When 組織コンテキストが切り替えられた場合, the ビジ友システム shall マイページ・案件管理・応募管理・メッセージ等のスタッフ向け画面が新しい組織のデータを表示するように再フェッチする。具体対象にはスカウトテンプレート（一覧・詳細・編集・送信画面）と求人お問い合わせ受信箱（一覧・詳細）も含み、いずれもアクティブ組織で絞る（2026-07-15 97022be で追加。E2E: `e2e/multi-org-data-scoping.spec.ts`）。
