...
- [x] 11.1 (P) ADM-016/017 お問い合わせ一覧・詳細を実装する
  ...
  - 一覧 `/admin/contacts`: 受信日時降順・20件・絞込なし。各行: 受信日時（formatDateTime）・会社名/屋号・氏名・お問い合わせ内容（inquiry_type）・**「登録ユーザー」バッジ（user_id あり時のみ。2026-09-18 に「ログイン時に送信」へ改称）**。検索: 会社名/屋号・氏名・メール（ilike）
...
- [x] 14. E2E テスト（Playwright）
- [x] 14.1 admin 導線スモークを作成する
