...
- 表示項目（デザインカンプ CLI-020.png に準拠）:
  ...
  - 基本情報（表形式）:
    ...
    - 募集エリア（`client_recruit_areas` 別テーブル。master-area Migration 4 で `client_profiles.recruit_area` カラムは DROP 済、エリアは正規化テーブルで管理）
...
- 編集可能フィールド（デザインカンプ CLI-020.png / CLI-021.png に準拠。入力順・必須マーク配置はカンプに従う）:
  ...
  - **募集職種（複数選択、master_trade_types 参照）** → `client_profiles.recruit_job_types`。会社として全般的に扱う職種（案件ごとの職種は `jobs.trade_types text[]` で別管理。master-skills 仕様）。※当初「必須」→ **2026-06-10 仕様変更⑤（2026-06-11 改訂）で setup（課金直後の初回設定）時のみ未入力可**。通常編集（CLI-021 edit）では従来どおり必須（ナッジは設けず、編集時の必須化で後からの入力を担保）。経緯は `.kiro/specs/admin/requirements.md`「関連する仕様変更⑤」、実装は billing spec Task 17
  - **募集エリア（複数選択、AreaListEditor で都道府県+市区町村）** → `client_recruit_areas` 別テーブル(master-area 正規化、Migration 4 で `client_profiles.recruit_area` カラム廃止)。書き込みは `replace_client_recruit_areas(p_client_id, p_areas jsonb)` RPC で全置換。※当初「必須」→ **2026-06-10 仕様変更⑤（2026-06-11 改訂）で setup 時のみ未入力可**（通常編集では必須）。同上
  - 従業員規模（任意、数値）→ `client_profiles.employee_scale`（プレースホルダー「100」、単位「人」）
  - 求める働き方（任意、OptionSets）→ `client_profiles.working_way`
  - 言語（任意、OptionSets、**複数選択可能**のマルチセレクトプルダウン）→ `client_profiles.language`（DB は「、」区切りの text で保存）
  - 発注者メッセージ（任意、複数行テキスト）→ `client_profiles.message`
  - 利用 SNS（任意、チェックボックス付きチップ型 UI、2 カラムグリッド）→ `client_profiles.sns_x` / `sns_instagram` / `sns_tiktok` / `sns_youtube` / `sns_facebook`
    - UI: CON-001-design-pc/sp.png を参考に、各 SNS を個別カード（`rounded-[8px] border` + 内部に Checkbox）で表示。選択時は `border-primary bg-primary/10` で視覚化。意味的には独立チェックボックスと同等（5 つの boolean 値）
...
- **プラン購入後の初回遷移（`?setup=true` モード、全プラン共通）**: 全プランの Stripe Checkout success_url を CLI-021?setup=true に統一する。旧 `/mypage/organization-setup` 暫定画面と個人・小規模プランの `/mypage?checkout=success` 遷移は廃止し、すべて CLI-021 に集約する。`/mypage/organization-setup` に直接アクセスされた場合も CLI-021 にリダイレクトする
  - **表示**: 画面上部にセットアップバナーを表示
    - 法人プラン: 「プラン登録が完了しました。社名を入力してください。その他の項目は後からいつでも編集できます」（billing Task 17・2026-06-11 で文言変更）
...
#### REQ-ORG-011: マイページ（CON-001）からの導線

本仕様で実装する CLI-016 / CLI-020 / CLI-022 への主要導線はマイページ（CON-001）の「情報を更新する」セクションに集約する。リンクの URL 誤記・表示条件漏れによる到達不能を防ぐため、以下のルールを明示する。

##### メニュー項目と遷移先（URL 完全一致必須）

| メニューラベル | 表示条件 | 遷移先 URL | 対応画面 |
|---|---|---|---|
| スカウトメッセージテンプレート一覧 | `isClient` | `/messages/templates` | CLI-016 |
| 発注者情報詳細 | `isClient` | `/mypage/client-profile` | CLI-020 |
| 担当者一覧 | `isCorporate` | `/mypage/members` | CLI-022 |

- 上記 URL は `src/app/(authenticated)/` 配下の実ファイルパスと**完全一致**させる。`/scouts/templates` / `/clients/profile` / `/organization/members` のような「REST 風で一見正しそうな URL」は誤記であり使用禁止（2026-04 実例で 3 リンクとも 404 を発生させた）
- 新規画面を追加した場合、本表と `src/app/(authenticated)/mypage/page.tsx` の `UPDATE_INFO_CLIENT_MENU` / `CORPORATE_ONLY_MENU` 定義を同時更新する

##### 表示条件 `isClient` / `isCorporate` の判定ルール

| ログインユーザーの `users.role` | `isClient` | `isCorporate` |
|---|---|---|
| `client` | 自分の `subscriptions.status IN ('active', 'past_due')` が真なら true | 上記が真 **かつ** `plan_type IN ('corporate', 'corporate_premium')` なら true |
| `staff` | 所属組織 Owner の subscription が上記条件を満たせば true | 同上 |
| `contractor` / `admin` / サブスク無し | false | false |

##### Staff ユーザーのプラン状態解決（表示用途のみ）

Staff は**自分の subscription を持たない**設計（Owner のサブスクに相乗り）。メニュー可視性判定等、Staff のプラン状態を参照する場合は以下の順で解決する:
...
ステップ 3 は RLS により Staff セッションから直接 SELECT 不可のため、**admin client** を使用する。`.eq('user_id', user.id)` のまま Staff に適用すると常に null になり、Staff に対して `isClient = false` となり発注者向け 3 メニューすべてが非表示になる（2026-04 実例）。
...
##### 「情報を更新する」セクションのロール別可視性（全メニュー）

`/mypage` の「情報を更新する」セクションに表示するメニューのロール別可視性を以下で規定する。既存の 3 共通メニュー（プラン変更・本人確認・ユーザープロフィール）と本 spec で追加した 3 メニューをまとめて明示する。

| メニューラベル | href | 無料受注者 | 個人/小規模発注者 | 法人 Owner | 法人 Admin / Staff |
|---|---|:-:|:-:|:-:|:-:|
| プラン変更 | `/billing` | ○ | ○ | ○ | ○（閲覧のみ、購入/変更ボタン非活性） |
| 本人確認・CCUS登録 | `/profile/verification` | ○ | ○ | ○ | **×** |
| ユーザープロフィール変更 | 動的（後述） | ○ | ○ | ○ | ○ |
| スカウトメッセージテンプレート一覧 | `/messages/templates` | × | ○ | ○ | ○ |
| 発注者情報詳細 | `/mypage/client-profile` | × | ○ | ○ | ○ |
| 担当者一覧 | `/mypage/members` | × | × | ○ | ○ |
| よくある質問 | `/faq` | ○ | ○ | ○ | ○ |
| お問い合わせ | `/contact` | ○ | ○ | ○ | ○ |

##### 「ユーザープロフィール変更」の遷移先（動的）

`users.role` に応じてリンク先を切り替える:

| ロール | 遷移先 | 理由 |
|---|---|---|
| `contractor` / `client`（個人・小規模・法人 Owner） | `/profile`（受注者向けプロフィール画面 → `/profile/edit` で編集） | 受注者として登録している人は registered/profile フローを通過済みでスキル・対応可能エリア・資格等のデータを持つため |
| `staff`（法人 Admin / Staff） | `/mypage/members/{自分のuserId}/edit`（CLI-024 自己編集モードに直リンク） | CLI-025 招待経由で作成された Admin / Staff は registered/profile フローを通っておらず `/profile` は空欄ページになる。また自己編集は仕様上 CLI-024 に一元化されている（L44 / L68 参照） |

##### マイページトップの表示項目（ロール別）

CON-001 の上部プロフィール領域に表示する要素のロール別可視性を以下で規定する。Staff（法人 Admin / Staff）は受注者向けデータ（スキル・資格・本人確認等）を**持たない**ため、これらを表示すると「未承認」等のミスリーディングな表示になる。該当要素は一律非表示。

| 表示要素 | 無料受注者 / 個人/小規模発注者 / 法人 Owner | 法人 Admin / Staff |
|---|:-:|:-:|
| アバター | ○（`users.avatar_url` or フォールバック icon） | ○（**所属組織 Owner の `client_profiles.image_url`（会社ロゴ）** or フォールバック icon。Admin/Staff は個人アバターを持たない設計のため、他ユーザーから見える発注者アバター（spec L291）と同一の「会社ロゴ」を mypage 自己表示にも使う） |
| 氏名 | ○ | ○ |
| 年齢（birth_date から算出） | ○（データあり時） | **×** |
| 対応職種（`user_skills` より） | ○（データあり時） | **×** |
| 経験年数（`user_skills.experience_years` より） | ○（データあり時） | **×** |
| 本人確認バッジ | ○ | **×** |
| CCUS バッジ | ○ | **×** |
| 自己紹介セクション（`users.bio`） | ○（データあり時） | **×**（防御的に role チェック追加） |
| 「プロフィールを変更する」ボタン遷移先 | `/profile` | `/mypage/members/{自分のuserId}/edit`（CLI-024 自己編集モード直リンク） |

**実装方針**: `isStaff = userData.role === 'staff'` を計算し、受注者向け要素を `{!isStaff && (…)}` でラップする。null チェック（bio=null, skillNames=null 等）は自動非表示を担保するが、role チェックも明示することで「Staff では絶対に表示しない」意図をコード上で明確化する。

##### Middleware による catch-all

Staff が直 URL（ブックマーク・旧導線・E2E 等）で `/profile`（閲覧ページ）または `/profile/edit` にアクセスした場合、Middleware で `/mypage/members/{自分のuserId}/edit` にリダイレクトする。実装は `src/middleware.ts` の `role === 'staff'` 分岐内。

##### CLI-023 の「もどる」ボタン遷移先（isSelf 分岐）

CLI-023（担当者詳細）の戻るボタンは、**表示中の詳細が自分か他人か**で遷移先を切り替える:

| 状況 | 戻り先 | 理由 |
|---|---|---|
| `user.id === targetId`（自分の詳細） | `/mypage` | mypage 上部の「プロフィールを変更する」ボタンおよび「ユーザープロフィール変更」メニューから CLI-024 → CLI-023 と遷移したケースの mental model に合わせる。自分の CLI-023 = 自分のプロフィール確認なので、論理的な親は mypage |
| `user.id !== targetId`（他メンバーの詳細） | `/mypage/members` | 他者の詳細は CLI-022 担当者一覧からドリルダウンして来たケース。論理的な親は担当者一覧 |

**実装**: `src/app/(authenticated)/mypage/members/[id]/page.tsx` で `const isSelf = user.id === id;` を計算し、`<BackButton href={isSelf ? "/mypage" : "/mypage/members"} />` で分岐。

**背景（再発防止用）**: Admin/Staff は mypage の 2 入口（「プロフィールを変更する」ボタン + 「ユーザープロフィール変更」メニュー）から CLI-024 自己編集 → CLI-023 → 戻る、という流れが典型動線。一律 `/mypage/members` に戻すと「編集し終わって自分の情報を見ていたのに、なぜか担当者一覧が出る」という意図外の遷移になるため。

- `pathname === '/profile'` → redirect
- `pathname === '/profile/edit'` または `/profile/edit/...` → redirect（既存）
- `/profile/verification` / `/profile/withdrawal` 等のサブパスは **redirect 対象外**（verification は Staff にも表示こそしないがアクセス自体は塞がない。withdrawal は `profile/withdrawal/actions.ts` 側で org_role != 'owner' を拒否済み）

##### 再発防止の回帰テスト

- `e2e/mypage-navigation.spec.ts` がロール別に「マイページ → メニュークリック → 画面到達」の通し検証を行う
- 新規メニュー項目を追加する際は同テストにも 1 ケース追加する
- Staff 固有のテスト: (1) 本人確認メニューが非表示、(2) ユーザープロフィール変更 → CLI-024 直遷移、(3) `/profile` 直 URL → CLI-024 リダイレクト
...
| 求める働き方(working_way） | ― | OptionSets | 複数選択（UI はマルチセレクトプルダウン、DB は text[] で保存） |
| 言語（language） | ― | OptionSets | 複数選択（UI はマルチセレクトプルダウン、DB は「、」区切り text で保存。既存データの「・」区切りは読み込み時に互換処理） |
