...
- 編集可能フィールド（デザインカンプ CLI-020.png / CLI-021.png に準拠。入力順・必須マーク配置はカンプに従う）:
  ...
  - **募集職種（必須、複数選択、master_trade_types 参照）** → `client_profiles.recruit_job_types`。会社として全般的に扱う職種（案件ごとの職種は `jobs.trade_types text[]` で別管理。master-skills 仕様）
  - **募集エリア（必須、複数選択、AreaListEditor で都道府県+市区町村）** → `client_recruit_areas` 別テーブル(master-area 正規化、Migration 4 で `client_profiles.recruit_area` カラム廃止)。書き込みは `replace_client_recruit_areas(p_client_id, p_areas jsonb)` RPC で全置換
