...
- Middleware: CON 系画面（/applications/history/）は認証済み全ロールにアクセス開放（CLAUDE.md のロール設計ルールに準拠）。CLI 系画面（/applications/received/, /applications/orders/）は発注者（client）・担当者（staff）のみ
- RLS:
  - SELECT（読み取り）:
    - 受注者: 自分の応募のみ閲覧可能
