...
| CLI-007 | 応募一覧（未対応） | 発注者 | mypage 導線。発注可否未回答（status = 'applied'）のみ表示する未対応インボックス |
| CLI-007B | 案件応募者一覧 | 発注者 | CLI-002 導線。指定案件スコープで全ステータスの応募者を表示（ステータスフィルタ付き） |
...
- 発注者（Client）: CLI-007, CLI-007B, CLI-008〜012, CLI-028
- 担当者（Staff）: 組織内の案件について CLI-007, CLI-007B, CLI-008〜012 を操作可能。CON-011〜013（応募履歴）はアクセス不可（staffは応募できないのでデータなし。Middlewareでブロック）
...
- デフォルトソート: 応募日の降順
- ページネーション: 18件ずつ（カード3列グリッドのため 3・2 の公倍数）
...
#### REQ-MT-004: 応募一覧（CLI-007） — **発注可否未回答のインボックス**

- **役割**: mypage「発注先を探す」セクションからの導線。**自社の案件に届いた `status = 'applied'`（発注可否未回答）の応募のみ**を表示する未対応インボックス。
- **WHERE 句**: `jobs.owner_id = user.id AND applications.status = 'applied'`（唯一の条件）。発注可否が決まった応募は CLI-010 に移るため本画面には表示されない。
...
- **カード表示項目**（上から順）:
  1. ステータスバッジ（カード左上）— 常に「応募あり」（applied 固定）
  2. アバター + 氏名
  3. 対応できる職種（カンマ区切り）
...
- ページネーション: 18件ずつ（カード3列グリッドのため 3・2 の公倍数）
- デフォルトソート: 応募日の降順
- **補足**: 案件単位で「全ステータスの応募者を俯瞰したい」場合は CLI-007B（案件応募者一覧）を使う

#### REQ-MT-004B: 案件応募者一覧（CLI-007B） — **案件スコープの全ステータス表示**

- **役割**: CLI-002（募集現場詳細）の「応募者をみる」ボタンからの導線。**指定案件に紐づく全ステータスの応募者**を俯瞰できる画面。`/jobs/[id]/applicants` ルートに配置。
- **WHERE 句**: `applications.job_id = :id`（案件スコープ。status は制限なし = 全ステータス表示）
- **認可**: 対象案件の owner_id、または案件の organization の組織メンバーのみ閲覧可。それ以外は `notFound()`。
- **UI は CLI-010 を流用**。ただし以下の差分:
  - **ヘッダー**: 「案件応募者一覧」 + 対象案件タイトルのバナーをページ上部に 1 回だけ表示
  - **カード内の案件情報インナーカードは非表示**（ページ全体で案件が固定されているため重複）
  - **ステータスフィルタ**: CLI-010 と同じコンポーネントを使用するが、**「応募あり（未対応）」を含む全カテゴリを選択可能**（`includeApplied={true}` variant）
  - **カードのアクションボタンはステータスで出し分け**:
    - `status === 'applied'` → 「応募詳細をみる」→ `/applications/received/:appId`（CLI-008）
    - それ以外 → 「発注内容詳細をみる」→ `/applications/orders/:appId`（CLI-011）
  - 「ユーザー詳細をみる」ボタンは CLI-010 と同じ（`/users/contractors/:uid`）
- ページネーション: 20件ずつ
- デフォルトソート: updated_at DESC
- **戻る**: CLI-002（`/jobs/[id]?manage=true`）へ戻る
- **スカウト経由バッジ**: `scout_message_id IS NOT NULL` の応募には CLI-010 と同じ紫系カプセルバッジを表示
...
#### REQ-MT-007: 発注履歴一覧（CLI-010） — **発注可否決定以降の管理ダッシュボード**

- **役割**: mypage「受発注の管理」セクションからの導線。**発注可否が決定した以降の応募（status ≠ 'applied'）**のみを表示する。
- **WHERE 句**: `jobs.owner_id = user.id AND applications.status IN ('accepted', 'completed', 'lost', 'cancelled', 'rejected')`。`applied` は CLI-007 側の役割なので本画面には含めない。
- ステータスフィルター（プルダウン即時反映、検索ボタンなし — CON-011 と同じ方式）。**「応募あり（未対応）」は CLI-010 では選択肢から除外**（`includeApplied={false}` variant）
...
発注者:
CON-001 → CLI-007（応募一覧・未対応のみ）→ CLI-008（応募詳細）→ CLI-009（発注可否）→ CLI-007
CON-001 → CLI-010（発注履歴一覧・発注可否決定以降）→ CLI-011（発注履歴詳細）→ CLI-012（完了報告・評価）
CLI-001 → CLI-002（募集現場詳細）→ CLI-007B（案件応募者一覧・全ステータス）→ CLI-008 or CLI-011
CLI-006（ユーザー詳細）→ CLI-028（発注者評価表示）

**ステータス分離のルール**:
- `applied` のみ → CLI-007（mypage インボックス）
- `applied` 以外の全ステータス → CLI-010（mypage 管理ダッシュボード）
- 案件スコープで全ステータス横断 → CLI-007B（CLI-002 からの導線）
- 新しい status 値を追加する場合、未決状態（発注者判断待ち）なら CLI-007 側、決着後なら CLI-010 側と CLI-007B 側に含める
```
...
- **バッジ表示**: 以下の画面で `scout_message_id IS NOT NULL` の応募に「スカウト経由」バッジを表示する。発注者側は応募段階（CLI-007/008）から発注確定後（CLI-010/011）まで、受注者側は応募履歴（CON-011/012）で、いずれもライフサイクル全体でスカウト経由であることが判別できるようにする
  ...
  - CLI-007（応募一覧・未対応）: ステータスバッジの横に「スカウト経由」バッジ
  - CLI-007B（案件応募者一覧）: ステータスバッジの横に「スカウト経由」バッジ
