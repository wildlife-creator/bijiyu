...
- [x] 1.2 (P) 案件の Zod バリデーションスキーマを作成する
  - 案件フォームの全入力項目（タイトル、案件詳細、職種、報酬上限・下限、勤務地、工期、募集期間、募集人数、その他任意項目）に対応するスキーマを定義する
  - 必須項目と任意項目を区別し、各フィールドに日本語のエラーメッセージを設定する
  - クロスフィールドバリデーション: 報酬上限 >= 下限、工期終了日 >= 開始日、募集終了日 >= 開始日
  - ステータスは draft / open / closed の3値のみ許可する
...
- [x] 5.2 詳細表示コンポーネントを実装する
  - 全表示項目を配置する: タイトル、案件詳細、職種、報酬（下限〜上限）、勤務地、工期、募集期間、稼働時間、経験年数、スキル、国籍・言語、持ち物、スケジュール詳細、請負案件詳細、発注者メッセージ、詳細その他
  - ステータスバッジを表示する
  - 急募フラグはオプション加入時のみ表示する（option_subscriptions を参照）
  - 添付画像・書類を一覧表示する（`<img>` タグで表示、next/image は使用しない）
  - 「編集する」ボタン → CLI-003（/jobs/[id]/edit）への遷移
  - 応募者数の表示と CLI-007（応募一覧）への導線リンクを配置する
  - デザインカンプに合わせたレイアウト・配置・余白を適用する
  - _Requirements: REQ-JP-002_
...
- [x] 6.1 JobForm コンポーネントを実装する
  ...
  - セクション分割: 基本情報（タイトル、詳細、職種）/ 報酬・勤務条件（報酬、勤務地、工期、募集期間、人数）/ 詳細情報（稼働時間、経験年数、スキル等の任意項目）/ 画像
...
- [x] 7.1 新規登録ページ（CLI-004）を実装する
  ...
  - フォーム送信時に createJobAction を呼び出し、成功時に CLI-002（/jobs/[id]）へリダイレクトする
  - 送信エラー時はトースト通知でエラーメッセージを表示する
  - _Requirements: REQ-JP-004_

- [x] 7.2 編集ページ（CLI-003）を実装する
  - Server Component で既存の案件データと画像を取得し、JobForm を edit モードで配置する
  - フォーム送信時に updateJobAction を呼び出し、成功時に CLI-002（/jobs/[id]）へリダイレクトする
  - ステータス変更UIを実装する: 現在のステータスに応じて遷移可能なボタンを表示する（draft → 「公開する」、open → 「締切にする」）
  - 送信エラー時はトースト通知でエラーメッセージを表示する
  - _Requirements: REQ-JP-003_
