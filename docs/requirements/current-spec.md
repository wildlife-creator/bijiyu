...
2. **基本プラン**: プラン比較表へのリンク、月払い / 年払いの切替タブ、4 プランの行。行のボタンは **「このプランにする」の 1 種類**（銀行振込中は「カード払いにする」）。契約中の行は「ご利用中」バッジのみ。予約中・支払い遅延中・担当者は押せない。
3. **オプションプラン**: 「購入済み」（動画プランの購入日つき一覧）→ 動画 3 プラン（価格 + 1 行の説明 + 「詳しく見る」でたたむ注意書き + 「申し込む」/「再度購入する」）→ 急募（価格・説明は常時。案件の選択と申込は有料のみ）→ 補償（販売中または加入中のときだけ）。
4. 銀行振込の案内 1 行、「もどる」。
