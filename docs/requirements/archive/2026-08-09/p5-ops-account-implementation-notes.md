...
> **2026-09-17 追記**: ADM-009 の「管理運営アカウント」設定セクション（`ops-account-panel.tsx` / `ops-account-actions.ts`）は廃止した。設定は開発側で行う（ADM-009 の「銀行振込」枠でハイエンドを有効化 → SQL で `users.is_hidden = true`）。非表示の仕組み・バッジ・messages の RLS はそのまま。以下の本文は P5 実装当時の記録。
