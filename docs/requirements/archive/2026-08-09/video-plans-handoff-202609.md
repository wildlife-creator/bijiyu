...
> **2026-09-18 追記**: 本メモで「新規販売停止・コードは残す」とした旧 職場紹介動画（`video_workplace`）は、リリース前で実購入者がテストユーザーのみと確認できたため**コードごと削除した**（migration `20260918120000_drop_video_workplace_option.sql` で既存行を `video` に書き換え）。以下の `video_workplace` に関する記述は履歴として残す。
...
| `video_shooting` | 20,000 | 全会員 | ユーザー撮影動画制作プラン | ユーザー撮影動画制作プラン | ユーザー撮影動画制作プラン | ご自身で撮影した動画を、ビジ友が編集して掲載することができます。※ビジ友で決められた動画の構成に合わせて動画撮影をお願いします。 |
...
**2. ユーザー撮影動画制作プラン　20,000 円　買える人: 全会員（staff 不可）**
...
| 6 | 公式SNS動画の条件 | 全会員（staff 不可）・作り直しの再購入可。**ADM-008 の絞り込みに「ユーザー撮影動画制作プラン」「ビジ友公式SNS動画」を追加**（ADM-008 の選択肢 = プロフィール動画 / ユーザー撮影動画制作プラン / ビジ友公式SNS動画 / 補償 2 種。ADM-003 は 急募 / プロフィール動画 のまま） |
...
- `BillingClient.tsx`: 職場紹介動画の行を削除、自己PR 行を「プロフィール動画制作プラン」に改名・説明文差し替え（§4.1 の 1 番。注意書き 2 つ含む）、`isClientPlanActive` 制限を外す。ユーザー撮影動画制作プランの説明文も §4.1 の 2 番へ差し替え
...
表の下にオプション価格表（急募 20,000 円（7日間）/ プロフィール動画制作プラン 100,000 円 / ユーザー撮影動画制作プラン 20,000 円 / ビジ友公式SNS動画制作プラン 120,000 円）。初回事務手数料は 20,000 → **12,000 円**。
...
- 変更 1: 動画オプション申込完了メール（会員宛）の結びを 3 プラン共通の「今後の進め方については、運営よりご連絡いたします。」に変更（旧「運営より動画制作・撮影手配についてご連絡いたします」はユーザー撮影動画制作プランに合わない）。プラン別分岐は設けない
