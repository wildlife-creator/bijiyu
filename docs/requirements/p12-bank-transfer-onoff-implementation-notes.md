...
| 一覧 | ADM-025 を「**銀行振込お問い合わせ一覧**」に作り替え。銀行振込のお問い合わせだけを新着順に並べる。**ステータスは持たない**。各行からお問い合わせ詳細へ（**2026-09-17 変更**: ユーザー詳細・発注者詳細への直リンクは削除。**2026-09-18 変更**: お問い合わせ詳細の「送信ユーザーの詳細を見る」も削除し、運営がユーザーアカウント一覧で検索して開く運用に。お問い合わせ詳細には「送信時のログインアカウント」を文字で表示） |
| 有効化 | ユーザー詳細（ADM-009）の「銀行振込」枠（**2026-09-17 変更**: 契約は会員に紐づくため、発注者詳細 ADM-004 の枠は削除。ADM-004 はプラン・支払い方法の表示だけ）。**プランのオン／オフ（と変更）だけ**。年払い・利用開始日・有効期限・期限延長・期限バッジ・期限メールは持たない |
...
3. 銀行振込を選んだときだけ「希望プラン」（必須・単一選択）を表示: ライト／スタンダード／プレミアム／ハイエンド／プロフィール動画制作プラン／ユーザー撮影動画制作プラン／ビジ友公式SNS動画制作プラン。他の種類を選んでいるときは表示せず、送信値も空
...
12. 行のボタン: 「お問い合わせ詳細」（ADM-017、`backTo` 付き）のみ（2026-09-17 変更。当初あった「ユーザー詳細 / 発注者詳細」ボタンは削除）
13. 管理メニュー（ADM-002）のラベルを「銀行振込お問い合わせ一覧」に変更。ADM-026・ADM-025-B は削除

### 2.2 ADM-017 お問い合わせ詳細
14. 希望プランがあれば「希望プラン」行を表示（既存の「ログイン時に送信」バッジ（2026-09-18 に「登録ユーザー」から改称）はそのまま。ユーザー詳細リンクは 2026-09-18 に削除し「送信時のログインアカウント」の文字表示に置き換え）

### 2.3 「銀行振込」枠（ADM-009 ユーザー詳細のみ。2026-09-17 に ADM-004 発注者詳細からは削除）

> **2026-09-18 追記**: 動画プランの欄に「購入済み:」の一覧（表示専用）を追加。カード・銀行振込の両方の購入記録を新しい順に「プラン名　購入日（支払い方法）」で 1 件 1 行。同じプランを複数回買えば複数行（作り直しの再購入は正当）。運営が二重の有効化に気づけるようにするためで、取り消し機能は持たない（誤って有効化したときは開発側で対応）。急募は引き続き銀行振込の対象外（対応するかはチームで検討中）。
表示条件: 対象が `role IN ('contractor','client')` かつ退会済みでない。staff / admin には出さない。（「管理運営アカウント」枠は 2026-09-17 に廃止）

15. **有料プランなし** → 基本プラン（Select、既定ライト）+「有効にする」。確認ダイアログ「入金を確認したうえで有効化してください。有料会員に切り替わり、本人に有効化メールが届きます」→ 実行。（当初は有効化後に「発注者詳細を開く」リンクを表示していたが、2026-09-17 に削除）
...
- E2E `bank-transfer.spec.ts`: ①会員ログイン → お問い合わせ（銀行振込・希望プラン）→ 送信 ②admin → 銀行振込お問い合わせ一覧に出る → ユーザー詳細 → 有効にする → 会員の /billing で「ご利用中」+「お支払い方法: 銀行振込」 ③未ログインで /contact に銀行振込の選択肢が無い ④ユーザー詳細から「変更する」「無効にする」（発注者詳細に枠が無いことも確認）。`admin.spec.ts` のメニュー名を更新
