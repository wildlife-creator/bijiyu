...
test.describe("CLI-026 表示: 未課金 contractor", () => {
  test("基本プランに「このプランにする」ボタンが4つ表示される", async ({ page }) => {
    ...
    const buttons = page.getByRole("button", { name: /円\/月 このプランにする$/ });
    await expect(buttons).toHaveCount(4);
    await expect(page.getByRole("button", { name: "2,800円/月 このプランにする" })).toBeVisible();
  ...
  test("「年払い」に切り替えると年額の申込ボタンに変わる", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "28,000円/年 このプランにする" })).toBeVisible();
  ...
  test("オプションプランセクションが表示される", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "ユーザー撮影動画を申し込む" }),
    ).toBeEnabled();
    ...
    await expect(
      page.getByRole("button", { name: "公式SNS動画を申し込む" }),
    ).toBeEnabled();
