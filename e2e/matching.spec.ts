...
test.describe("発注者: 作業完了報告・評価登録（CLI-012, 7項目★×5）", () => {
  test("総合評価のみ必須・任意項目／道具の『該当なし』も選んで登録できる", async ({
    page,
  }) => {
    await login(page, TEST_CLIENT.email, TEST_CLIENT.password);
    ...
    // 道具2項目は「該当なし」を選択（評価不能 → 集計から除外。補足文言も確認）
    await expect(
      page
        .getByText("評価が難しい場合は「該当なし」を選んでください")
        .first(),
    ).toBeVisible();
    const naButtons = page.getByRole("button", { name: "該当なし" });
    await expect(naButtons).toHaveCount(2);
    await naButtons.nth(0).click();
    await naButtons.nth(1).click();
    await expect(naButtons.nth(0)).toHaveAttribute("aria-pressed", "true");
    await expect(naButtons.nth(1)).toHaveAttribute("aria-pressed", "true");
