...
test.describe("master-area-multi-select Phase E: 既存データ正規化 (シナリオ 5)", () => {
  test("contractor2 (seed で 東京都全域 + 港区 + 新宿区 混在) が /profile/edit を開くと 東京都全域 1 行に正規化される", async ({
    page,
  }) => {
    // seed.sql: cc111111 (contractor2) は user_available_areas に
    //   (東京都, NULL), (東京都, 港区), (東京都, 新宿区), (神奈川県, NULL)
    // を持つ。collapseAreasFromDb は同県内 NULL 混在を「県全域優先」で正規化するため、
    // /profile/edit を開くと 東京都行は 全域 Checkbox ON / 港区・新宿区 はチェックされない状態
    // (混在の具体 muni を捨てる、Req 5-2 / 12-3) で defaultValues に注入される。
    ...
    // ロード完了を待つ (collapseAreasFromDb が走ったあとの React render)
    await page.waitForLoadState("networkidle");

    // 「全域」Checkbox が 1 つ以上 ON 状態 (東京都 / 神奈川県 のどちらか)
    const wholeChecked = page.getByLabel("全域", { exact: true }).first();
    await expect(wholeChecked).toBeChecked();

    // 港区・新宿区 の Checkbox がチェックされていない(全域優先で捨てられた)
    const portRegionBox = page.getByLabel("港区", { exact: true });
    if ((await portRegionBox.count()) > 0) {
      await expect(portRegionBox.first()).not.toBeChecked();
    }
  });
});
