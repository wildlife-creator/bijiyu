 ...
 * master-skills.spec.ts / client-profile.spec.ts の AreaPicker 操作
 * によって既にカバーされているため、本ファイルでは省略する。
 *
 * 命名: shadcn Select 操作は `getByLabel()` ベースでなく、AreaPicker が
 * 描画する Select trigger を `[data-slot="select-trigger"]` で取得する。
 * 詳細は master-skills.spec.ts Phase 4 commit のコメント参照。
 */
...
test.describe("master-area: カード「他Nエリア」省略表示 (シナリオ 9)", () => {
  test("案件カードでエリア 4 件以上の案件は「他Nエリア」と表示される", async ({
    ...
    await login(page, TEST_CONTRACTOR.email);
    // seed の「東京都 大型マンション新築 大工工事」は job_areas を 4 件持つ
    // (東京都港区/世田谷区/品川区 + 神奈川県横浜市西区)
...
test.describe("master-area: 案件エリア入力 10 件上限 (シナリオ 11)", () => {
  test("案件作成画面で「+ エリアを追加」ボタンは 10 件到達で disabled", async ({
    page,
  }) => {
    await login(page, TEST_CLIENT.email);
    await page.goto("/jobs/create");

    // 初期状態で 1 件目の AreaPicker が表示されている
    // 「+ エリアを追加」を 9 回押すと 10 件に到達
    const addButton = page.getByRole("button", { name: "+ エリアを追加" });
