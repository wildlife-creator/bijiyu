...
test.describe("管理者: 動画投稿導線（ADM ログイン → 一覧 → 詳細 → 投稿）", () => {
  ...
  test("ダッシュボード → ユーザー一覧 → 詳細 → ADM-010 で URL 登録できる", async ({
    ...
    // active 'video' があるので「受注者PR動画を投稿する」ボタンが出る
    await page
      .getByRole("link", { name: "受注者PR動画を投稿する" })
      .click();
  ...
  test("ADM-009 で職場紹介動画オプション加入者には職場紹介投稿ボタンが出る", async ({
    page,
  }) => {
    await login(page, TEST_ADMIN.email, TEST_ADMIN.password);
    ...
    await expect(
      page.getByRole("link", { name: "職場紹介動画を投稿する" }),
    ).toBeVisible();
    // ADM-010B へ遷移して空更新（掲載停止）
    await page.getByRole("link", { name: "職場紹介動画を投稿する" }).click();
