...
### 管理運営アカウント（users.is_hidden、必ず守ること）
...
### 一覧の並び替え（プラン順ランク列 + 共通プルダウン、必ず守ること）
- 発注者一覧（CON-005）と案件一覧（CON-002 おすすめ順）のプラン順は **`users.list_plan_rank` / `jobs.owner_plan_rank`（0 = その他（無料 / ライト）/ 1 = スタンダード / 2 = プレミアム / 3 = ハイエンド）** で並べる。ランクは `subscriptions` / `jobs` / `organizations` のトリガーで自動更新される（`20260902140000_list_plan_rank.sql` + `20260910130000_list_plan_rank_small.sql`）。**TS 側で都度ランクを更新するコードや、subscriptions を join して並べるクエリを書かないこと**（subscriptions の RLS は本人行のみで他人のランクが 0 になる）
...
- Webhook・解約・メールはフラグに関係なく動かす（既存契約のため）。管理画面 ADM-008 の絞り込みに補償は含めない（加入者がいないため。復活させるときは `users/page.tsx` の `VALID_OPTIONS` と `users/filters.tsx` に戻す）。補償関連コードを「使われていない」と判断して削除しないこと
...
### 銀行振込は「お問い合わせ → 運営がオン／オフ」だけ（必ず守ること）
- 銀行振込の申込テーブル（`bank_transfer_requests`・申込受付→請求書送付済→入金確認済）・運営による代理登録・ADM-026・本人申込ボタン（`NEXT_PUBLIC_BANK_TRANSFER_SELF_SERVICE_ENABLED`）・年払い/利用開始日/有効期限/期限延長/期限バッジ/期限メール（Edge Function `bank-transfer-expiry-notify`）は**持たない（廃止済み）**。正の仕様は `docs/requirements/current-spec.md` §2.2。**これらを復活させないこと**
- 入口は **お問い合わせ（COM-008）の「お支払い方法（銀行振込）について」**。`BANK_TRANSFER_INQUIRY_TYPE`（`src/lib/constants/contact-options.ts`）は**ログイン中だけ**選べる（フォームは未ログインに選択肢を出さず、`submitContactAction` でも未ログインなら拒否）。選ぶと「希望プラン」（`contacts.bank_transfer_plan`、`BANK_TRANSFER_PLAN_CHOICES` のキー = 基本プラン 4 + 動画プラン 3）が必須になる。ログイン中は種類に関係なく会社名・氏名・メール・所在地を会員情報から初期入力する（`resolveContactPrefill`）
- **ADM-025 銀行振込お問い合わせ一覧（`/admin/bank-transfers`）は `contacts` を `inquiry_type = BANK_TRANSFER_INQUIRY_TYPE` で絞った一覧**（`fetchBankTransferContactList`、`src/lib/admin/bank-transfers.ts`）。ステータスは持たない。各行のリンクは「お問い合わせ詳細」（ADM-017）だけ。**ADM-017 にはユーザー詳細への直リンクを置かない（全種類のお問い合わせ）**: 複数アカウントを持つ会員が別アカウント宛ての依頼を送ることがあり、ボタンで開いた相手をそのまま有効化すると取り違えるため。運営は ADM-008 で検索して確認する。代わりに ADM-017 上部に「送信時のログインアカウント：（登録メール）」を文字だけで出し、フォームのメールと違えば「※ フォームのメールアドレスと異なります」を出す。お問い合わせ → ユーザー詳細の直リンクを足し直さないこと
- **有効化・変更・停止は共通部品 `<BankTransferPanel>`**（`src/components/admin/bank-transfer-panel.tsx`）で、**ADM-009 ユーザー詳細だけに置く**（契約は会員に紐づくため。ADM-004 発注者詳細はプラン・支払い方法の表示のみ。ADM-004 に枠や ADM-009 へのリンクを足し直さないこと）。Server Action は `src/app/admin/(protected)/clients/[id]/bank-subscription-actions.ts` に集約: `activateBankTransferPlanAction`（有効プランなし → `grantBankTransferPlan` で bank_transfer 行を作る。`current_period_end` は NULL = 期限なし）/ `changeBankSubscriptionPlanAction`（変更）/ `cancelBankSubscriptionAction`（無効化 = Stripe 解約と同じ後処理）/ `switchStripeToBankTransferAction`（カード → 銀行振込。§下記）/ `activateBankTransferVideoOptionAction`（動画プラン。`option_subscriptions` に one_time / bank_transfer 行 + `sendVideoActivatedEmails`）。対象は `role IN ('contractor','client')` かつ退会済みでない会員のみ
- **支払い方法の切り替えは同じ契約行の書き換えで行い、運営が順番を気にしなくてよい形にしている**（`subscriptions_unique_active` は据え置き。「2 行同時に有効」を許すコードを書かないこと）:
  - カード → 銀行振込: `switchStripeToBankTransferAction` が Stripe の schedule を release → `stripe.subscriptions.cancel`（即時・日割り返金なし）→ 同じ行を `payment_method='bank_transfer'` / `stripe_subscription_id=NULL` / 予約系 NULL / `current_period_end=NULL` に UPDATE。その後届く `customer.subscription.updated / deleted` は行が見つからないので Webhook 側が黙って skip する（`handle-subscription-lifecycle.ts` の既存挙動。壊さないこと）
  - 銀行振込 → カード: 会員が料金プラン画面の「カード払いにする」で Checkout（`startCheckoutAction` の DB ガードは **Stripe 行だけ**を見る）。`handle_checkout_completed_plan` v3（`20260916120000`）が有効な bank_transfer 行を後処理なしで `cancelled` にしてから Stripe 行を INSERT（監査 `bank_transfer_ended_by_stripe_checkout`）
- 銀行振込行の `billing_cycle` は `monthly` 固定、`current_period_end` は NULL（期限なし）。**bank_transfer 行の `current_period_end` を表示・判定に使わないこと**（管理運営アカウントも同じ銀行振込行で期限なし）
- 補償オプション（販売停止中）と急募は銀行振込の対象外。FAQ Q17 は「ログインのうえお問い合わせ窓口から」（`docs/legal/faq.md` と `(support)/faq/content.ts` は一字一句一致の vitest あり）
- E2E: `e2e/bank-transfer.spec.ts`（会員のお問い合わせ → 一覧 → ユーザー詳細で有効化 → /billing、ユーザー詳細で変更・動画プランの有効化・無効化（発注者詳細には枠が無いことも確認）、未ログインは選択肢なし）
...
- **動画商品（`docs/requirements/current-spec.md` §3）**: 商品は「プロフィール動画制作プラン（`video`、10 万）/ ユーザー撮影動画制作プラン（`video_shooting`、2 万）/ ビジ友公式SNS動画制作プラン（`video_sns`、12 万）」の 3 つ。旧「職場紹介動画」（`video_workplace`）は `video` に統合し、**キーごと削除済み**（migration `20260918120000_drop_video_workplace_option.sql` が既存行を `video` に書き換え。`isDiscontinuedOption` / `PROFILE_VIDEO_OPTION_TYPES` / `STRIPE_PRICE_VIDEO_WORKPLACE` も無い。復活させないこと）。全会員（staff 以外）が購入可で「発注者プラン加入者のみ」のガードは撤廃済み。**プレミアム・ハイエンドへの付属（10 万は全員、12 万は年払いのみ）はアプリで判定しない**（料金画面の注意書きと ADM-004 のプラン + 支払サイクル表示で運用。ユーザー判断: 確認箇所を増やさない）。付属判定・決済ガード・admin バッジを勝手に足さないこと。会員が見る動画欄の見出しは掲載先に関係なく `VIDEO_SECTION_LABEL`（「プロフィール動画」）、ADM-027 のタブ名は画面名（`VIDEO_PLACEMENT_LABELS`）、掲載お知らせメールは【掲載先】（`VIDEO_PLACEMENT_MEMBER_LABELS`）。**ADM-008 の絞り込みは動画 3 プラン（正式名 = `VIDEO_OPTION_UI_NAMES`）のみ、ADM-003 の絞り込み・行バッジと ADM-004 のオプション加入状況は急募のみ**（ADM-003 / ADM-004 に動画の絞り込み・バッジ・チェックを足し直さないこと）
- **買い切り動画系オプション（`video` / `video_shooting` / `video_sns`）を足すときの触り方**: `src/lib/billing/options.ts` の `OptionType` / `OPTION_LABELS` / `OPTION_PRICES_TAX_INCLUDED` / `VIDEO_OPTION_TYPES` に追加 → Checkout（`billing/actions.ts` の Zod スキーマ・`priceIdForOption`・`buildSuccessUrl`）→ 銀行振込の希望プラン・有効化対象（`src/lib/constants/contact-options.ts` の `BANK_TRANSFER_PLAN_CHOICES`。お問い合わせの選択肢と管理画面の動画有効化 Select が同じ定数を使う）→ 料金画面（`BillingClient.tsx` の行 + `hasVideoOption` + `VIDEO_OPTION_UI_NAMES`）→ ADM-008 の絞り込み（`users/page.tsx` の `VALID_OPTIONS` + `users/filters.tsx`）→ `.env.local.example` に `STRIPE_PRICE_*`。Webhook（`handleVideoOption`）・銀行振込の動画有効化（`activateBankTransferVideoOptionAction`）・メール（`sendVideoActivatedEmails`）は `isVideoOption()` / `VideoOptionType` 経由で自動的に対応する。`option_subscriptions.option_type` は CHECK 無しの text（DB 側に列挙は無い）

### 銀行振込（payment_method = 'bank_transfer'）と Stripe 前提処理の分離（必ず守ること）
- `subscriptions` / `option_subscriptions` は `payment_method`（stripe / bank_transfer）を持つ。銀行振込行は **Stripe にサブスクが存在しない**（`stripe_subscription_id` / `stripe_payment_intent_id` が NULL、CHECK 制約で保証）
...
- 銀行振込契約の有効化・プラン変更・無効化・カードからの切り替えは管理画面（ADM-009 の `<BankTransferPanel>`）の Server Action で行う。契約行の作成は Stripe 経路の RPC `handle_checkout_completed_plan` と同じ副作用（role 昇格・client_profiles・`ensure_organization_exists`・監査ログ・有効化メール）を `grantBankTransferPlan()`（`src/lib/billing/grant-plan.ts`）が再現している。副作用を追加する場合は **両経路に入れる**こと
- 有効化メール等の送信ヘルパーは `src/lib/billing/activation-emails.ts` に共通化済み（Webhook と管理画面の両方から呼ぶ）。Webhook ファイル内に新しい送信関数を閉じ込めないこと
- 期限は管理しない。銀行振込行は運営が「無効にする」まで有効。`expire-options` cron は銀行振込の補償を対象外にしたまま（既存契約の保護）
- 詳細: `docs/requirements/current-spec.md` §2.2、`.kiro/steering/database-schema.md`「銀行振込の運用」
...
- **メールアドレス等の「他の seed と前方が重複しうる識別子」で行を特定する getByRole(link, name) は negative lookbehind で固定する（必ず守ること）**: 一覧画面の行リンクを `getByRole("link", { name: /xxx@test\.local/ }).first()` で掴むパターンは、後から seed に `adm-del-xxx@test.local` 等のサフィックス違いユーザーが増えると **substring 一致で巻き込まれ、`.first()` がアルファベット順で別人を掴む**。対策: 直前に `-` などが付かないことを negative lookbehind で要求する → `/(?<!-)xxx@test\.local/`。完全一致 `/^xxx@test\.local$/` ではリンクの accessible name が「氏名（N歳） email ›」のような複合テキストなので何も match しなくなる点に注意。2026-06-18 実例: ADM-009 マージで `adm-del-contractor@test.local` が追加され、`e2e/video-display.spec.ts:131` の「ダッシュボード → ユーザー一覧 → 詳細 → ADM-010」テストが「受注者PR動画を投稿/編集する」ボタンの timeout で fail（退会済みユーザー詳細を掴んでいた）
- **パスワード入力欄は `getByRole("textbox", { name: /パスワード/ })` で掴む（必ず守ること）**: `<PasswordInput>` (src/components/ui/password-input.tsx) は表示トグル `<button aria-label="パスワードを表示">` を内包する。Playwright の `getByLabel("パスワード")` は **substring match** なので、トグルボタンの aria-label "パスワードを表示" も巻き込んで strict mode violation で fail する（`exact: true` も `/login` は label が "パスワード 必須"・`/admin/login` は "パスワード" と非対称なため両方の login 画面では使えない）。対策: `<input type="password">` の implicit role が `textbox`、トグルは `role=button` なので **`getByRole("textbox", { name: /パスワード/ })` で正しく入力欄だけ掴める**。同名で `（確認）` 等のサフィックスが付く場合は `exact: true` も併用可（既存 `e2e/admin.spec.ts:332-333` のパターン）。2026-06-24 実例: f77ad9a (#22) でトグル追加後、`e2e/helpers.ts` の `login` ヘルパー含む 4 箇所がこの罠で全 E2E の ~80% を fail させていた
...
- **子テーブル (job_images 等) の RLS も `is_same_org` で組織対応させる**: `jobs.owner_id = auth.uid()` だけを条件にしている INSERT/UPDATE/DELETE ポリシーは、組織メンバーが「他メンバーが作った同組織案件」を扱えないため必ず `owner_id = auth.uid() OR (organization_id IS NOT NULL AND is_same_org(auth.uid(), organization_id))` に揃える。基準実装: `jobs.jobs_update` / `job_images` 全 DML ポリシー（`20260707130000_job_images_org_aware_rls.sql`）。**2026-07-07 実例**: `job_images` の INSERT/UPDATE/DELETE が Owner 限定になっていて、Staff 作成案件に Owner が画像を追加しようとしても Storage には書けるが `job_images` INSERT が silent block されて画像が公開画面に出ない現象。Server Action の insert に error 検査が無かったため画面上は成功したように見えた。今後 `jobs` に紐づく子テーブル (attachments / images / videos 等) を新設する際は最初から org 対応 RLS で書くこと
- **`.delete()` の silent RLS block を検出するには `.select()` で影響行を返してもらう**: PostgREST の DELETE は RLS で行がゼロなら error を返さない (影響 0 行で成功扱い)。UI 側で「削除された」トーストを出しつつ実データが残る症状の原因になる。`await supabase.from(x).delete().eq(...).select("id")` で戻りを確認し、`deletedRows.length === 0` なら権限不足として明示的にエラーを返すこと。基準実装: `deleteJobImageAction` (`src/app/(authenticated)/jobs/actions.ts`)
...
### アップグレードは Stripe ホスト画面で確定、DB 更新とメールは Webhook 側（必ず守ること）
- アップグレード（上位プラン / 月払い→年払い）は `changePlanAction` が Stripe Customer Portal の `subscription_update_confirm` セッション URL を返し、ユーザーが Stripe 側で確定する。**Server Action は DB を先行 UPDATE せず、完了メールも送らない**（ホスト画面で離脱・決済失敗した場合に DB だけ進む事故を防ぐ）
...
- **プラン変更ポータルの `proration_behavior` は `always_invoice`**（`scripts/stripe/setup-yearly-prices.mjs`）。`create_prorations` だと日割りの明細を作るだけで請求は次回更新日にまとめられ、月払い→年払いの会員は **1 年間追加請求なし**、上位プランの差額も翌月合算になる（確認画面の「本日のご請求」と実際が食い違う）。2026-09 支払い E2E で Stripe テストクロックで実時間追跡して確定。設定を作り直す・本番に作るときは必ず `always_invoice` を確認する

### Stripe Webhook の到着順・遅延に強くする（2026-09 支払い E2E 全パターンで見つけた 3 件、必ず守ること）
- **ダウングレード予約は Server Action で先行 UPDATE + 予約メールを同期送信**（`scheduleDowngradeAction`。**予約メールは Server Action だけが送る。Webhook (b) 分岐で送ると処理順によって 2 通届く** — 2026-09-24 再確認 E2E で実例）。`subscriptionSchedules.create` → `update`（第 2 フェーズ書き込み）の 2 回呼びで、`customer.subscription.updated` は create の時点で出るため、Webhook が update より先に処理されると `schedule_id` だけ入って `scheduled_plan_type` が NULL のまま確定し、料金画面が「変更予定」も取消ボタンも出ない行き止まりになる（28 回中 10 回再現）。Webhook 側（`resolveScheduleNextPhase` の `kind: "pending"`）も「次フェーズ未確定」なら既存の DB 値を維持する両面対策。**この 2 段呼びを 1 回にまとめる、または片側の対策だけ外す変更をしないこと**
- **期末に予約が適用されたら予約カラムを消し、Stripe のスケジュールを release する**（`kind: "applied"` = `current_phase` が最後のフェーズ）。既定の `end_behavior=release` だと適用後も第 2 フェーズとしてスケジュールが 1 か月残り、アプリは `schedule_id` がある限り「予約中」扱いにするため「変更予定」表示と他プランのロックが適用後も残り続ける。`resolveScheduleNextPhase` は `phases[1]` 固定ではなく **`current_phase` の次**を見ること
- **DB が `cancelled` の契約行は `customer.subscription.updated` で巻き戻さない**（`handleSubscriptionUpdated` 冒頭のガード）。`cancelImmediatelyAction` が cancel 直前に metadata を書くため `updated(status=past_due)` が発生し、`deleted` の処理より後に DB を書くと cancelled → past_due に戻って「Stripe は解約済み・role は受注者なのに `is_paid_user()` = true」になる。Stripe の契約は解約後に復活しないので、cancelled 行への updated は常に無視してよい
- 回帰テスト: `src/__tests__/billing/webhook/handle-subscription-lifecycle.test.ts`「2026-09 支払い E2E で見つかった不具合の回帰」/ `plan-actions.test.ts`「ダウングレード予約: 予約内容を DB に先行 UPDATE…」。実機の全パターン確認は `scripts/capture/output/e2e-report/run-pay-full.mjs`（gitignore 配下・Stripe テストクロック方式）
...
- **URL を SSOT にするフィルターフォームは、URL 由来の初期値を `key` にして内部 state を作り直す**: `useState(initialKeyword)` は「マウント時に一度だけ」写すため、ブラウザの戻る/進むで URL が変わっても入力欄が追従しない。`export function X(props) { return <XInner key={`${props.initialA}|${props.initialB}`} {...props} /> }` の形にする（admin の `filters.tsx` / `KeywordSearchForm` が基準）。useEffect で setState する形は避ける
- **`backTo` リレー（admin）は、URL を組み直す全経路（検索・ページ送り・並び替え）と全子リンクで維持する**: 1 箇所落とすと「もどる」がフォールバック先へ飛ぶ。新しい一覧・詳細を admin に足すときは `resolveBackTo` / `buildBackToValue` を使い、検索フォームには `backTo` prop を渡すこと
- **入力欄はスマホで 16px 以上**（詳細は design-rule.md「入力欄の文字サイズはスマホで 16px 以上」）。生の `<input>` / `<textarea>` / cmdk Input に `text-sm` / `text-body-*` を単独で当てない
- **Radix Dialog / Sheet で開いた直後にキーボードを出したくない場合は `onOpenAutoFocus={(e) => e.preventDefault()}`**（検索条件パネルは `SearchFilterSheet` で一括対応済）
...
- **E2E 全件の前に必ず `supabase db reset`**: 使い捨て seed（承認待ちの本人確認、銀行振込のお問い合わせ、削除対象の担当者、取消対象の応募等）は 1 回の実行で消費される。リセットせず再実行すると数十件が「要素が見つからない」で落ち、原因の切り分けに時間を浪費する（2026-09-08 に 36 件 fail の実例）
