...
- 課金は手動サブスク行（`payment_method='bank_transfer'`, `plan_type='corporate_premium'`, 期限 2099-12-31）。新しい支払方法 enum を足さないこと。付与は `grantBankTransferPlan()`（`src/lib/billing/grant-plan.ts`、P12 の銀行振込有効化 `activateBankTransferPlanAction` と共通）。有効化メールは送らない
...
- **ADM-025 銀行振込お問い合わせ一覧（`/admin/bank-transfers`）は `contacts` を `inquiry_type = BANK_TRANSFER_INQUIRY_TYPE` で絞った一覧**（`fetchBankTransferContactList`、`src/lib/admin/bank-transfers.ts`）。ステータスは持たない。各行から ADM-017 と ユーザー詳細（発注者なら ADM-004、それ以外は ADM-009）へ
- **有効化・変更・停止は共通部品 `<BankTransferPanel>`**（`src/components/admin/bank-transfer-panel.tsx`）で、ADM-009 と ADM-004 の両方に置く。Server Action は `src/app/admin/(protected)/clients/[id]/bank-subscription-actions.ts` に集約: `activateBankTransferPlanAction`（有効プランなし → `grantBankTransferPlan` で bank_transfer 行を作る。`current_period_end` は NULL = 期限なし）/ `changeBankSubscriptionPlanAction`（変更）/ `cancelBankSubscriptionAction`（無効化 = Stripe 解約と同じ後処理）/ `switchStripeToBankTransferAction`（カード → 銀行振込。§下記）/ `activateBankTransferVideoOptionAction`（動画プラン。`option_subscriptions` に one_time / bank_transfer 行 + `sendVideoActivatedEmails`）。対象は `role IN ('contractor','client')` かつ退会済みでない会員のみ
...
- 銀行振込行の `billing_cycle` は `monthly` 固定、`current_period_end` は NULL（期限なし）。**bank_transfer 行の `current_period_end` を表示・判定に使わないこと**。管理運営アカウント（P5、2099-12-31）だけは従来どおり
- 補償オプション（販売停止中）と急募は銀行振込の対象外。FAQ Q17 は「ログインのうえお問い合わせ窓口から」（`docs/legal/faq.md` と `(support)/faq/content.ts` は一字一句一致の vitest あり）
- E2E: `e2e/bank-transfer.spec.ts`（会員のお問い合わせ → 一覧 → ユーザー詳細で有効化 → /billing、発注者詳細で変更・無効化、未ログインは選択肢なし）
...
- 銀行振込契約の有効化・プラン変更・無効化・カードからの切り替えは管理画面（ADM-009 / ADM-004 の `<BankTransferPanel>`）の Server Action で行う。契約行の作成は Stripe 経路の RPC `handle_checkout_completed_plan` と同じ副作用（role 昇格・client_profiles・`ensure_organization_exists`・監査ログ・有効化メール）を `grantBankTransferPlan()`（`src/lib/billing/grant-plan.ts`）が再現している。副作用を追加する場合は **両経路に入れる**こと
