...
- 発注者一覧（CON-005）と案件一覧（CON-002 おすすめ順）のプラン順は **`users.list_plan_rank` / `jobs.owner_plan_rank`（0 = その他（無料 / ライト）/ 1 = スタンダード / 2 = プレミアム / 3 = ハイエンド。P11 でスタンダードを追加）** で並べる。ランクは `subscriptions` / `jobs` / `organizations` のトリガーで自動更新される（`20260902140000_list_plan_rank.sql` + `20260910130000_list_plan_rank_small.sql`）。**TS 側で都度ランクを更新するコードや、subscriptions を join して並べるクエリを書かないこと**（subscriptions の RLS は本人行のみで他人のランクが 0 になる）
...
- Webhook・解約・メールはフラグに関係なく動かす（既存契約のため）。管理画面 ADM-008 の補償の絞り込みは 2026-09-18 に削除した（加入者がいないことをユーザーが確認済み。復活させるときは `users/page.tsx` の `VALID_OPTIONS` と `users/filters.tsx` に戻す）。補償関連コードを「使われていない」と判断して削除しないこと
...
### 銀行振込は「お問い合わせ → 運営がオン／オフ」だけ（P12、必ず守ること）
- 2026-09-16 P12 で、P2 の申込テーブル（`bank_transfer_requests`・申込受付→請求書送付済→入金確認済）・P9 の代理登録（ADM-025-B）・ADM-026・本人申込ボタンと `NEXT_PUBLIC_BANK_TRANSFER_SELF_SERVICE_ENABLED`・年払い/利用開始日/有効期限/期限延長/期限バッジ/期限メール（Edge Function `bank-transfer-expiry-notify`）を**すべて廃止**した。正の仕様は `docs/requirements/p12-bank-transfer-onoff-implementation-notes.md`。**これらを復活させないこと**
...
- **支払い方法の切り替えは同じ契約行の書き換えで行い、運営が順番を気にしなくてよい形にしている**（`subscriptions_unique_active` は据え置き。「2 行同時に有効」を許すコードを書かないこと）:
  ...
  - 銀行振込 → カード: 会員が料金プラン画面の「カード払いで申し込む」で Checkout（`startCheckoutAction` の DB ガードは **Stripe 行だけ**を見る）。`handle_checkout_completed_plan` v3（`20260916120000`）が有効な bank_transfer 行を後処理なしで `cancelled` にしてから Stripe 行を INSERT（監査 `bank_transfer_ended_by_stripe_checkout`）
...
- **動画商品の整理（P10、2026-09。`docs/requirements/video-plans-handoff-202609.md` §4 が確定仕様）**: 商品は「プロフィール動画制作プラン（`video`、10 万）/ ユーザー撮影動画制作プラン（`video_shooting`、2 万）/ ビジ友公式SNS動画制作プラン（`video_sns`、12 万）」の 3 つ。旧「職場紹介動画」（`video_workplace`）は `video` に統合し**新規販売停止**（`isDiscontinuedOption()` で Checkout を拒否。P12 の銀行振込の動画有効化（`BANK_TRANSFER_PLAN_CHOICES`）にも含めない。既存行の Webhook・表示は維持するのでコードもキーも削除しない）。全会員（staff 以外）が購入可で「発注者プラン加入者のみ」のガードは撤廃済み。**プレミアム・ハイエンドへの付属（10 万は全員、12 万は年払いのみ）はアプリで判定しない**（料金画面の注意書きと ADM-004 のプラン + 支払サイクル表示で運用。ユーザー判断: 確認箇所を増やさない）。付属判定・決済ガード・admin バッジを勝手に足さないこと。会員が見る動画欄の見出しは掲載先に関係なく `VIDEO_SECTION_LABEL`（「プロフィール動画」）、ADM-027 のタブ名は画面名（`VIDEO_PLACEMENT_LABELS`）、掲載お知らせメールは【掲載先】（`VIDEO_PLACEMENT_MEMBER_LABELS`）。管理画面 ADM-008 の絞り込み「プロフィール動画制作プラン」は `PROFILE_VIDEO_OPTION_TYPES`（`video` + `video_workplace`）で両方にヒットさせる。**ADM-008 の絞り込みは動画 3 プラン（正式名 = `VIDEO_OPTION_UI_NAMES`）のみ、ADM-003 の絞り込み・行バッジと ADM-004 のオプション加入状況は急募のみ**（2026-09-18 整理。ADM-003 / ADM-004 に動画の絞り込み・バッジ・チェックを足し直さないこと）
- **買い切り動画系オプション（`video` / `video_workplace` / `video_shooting` / `video_sns`）を足すときの触り方（P7 で統合済み）**: `src/lib/billing/options.ts` の `OptionType` / `OPTION_LABELS` / `OPTION_PRICES_TAX_INCLUDED` / `VIDEO_OPTION_TYPES` に追加 → Checkout（`billing/actions.ts` の Zod スキーマ・`priceIdForOption`・`buildSuccessUrl`）→ 銀行振込の希望プラン・有効化対象（`src/lib/constants/contact-options.ts` の `BANK_TRANSFER_PLAN_CHOICES`。お問い合わせの選択肢と管理画面の動画有効化 Select が同じ定数を使う）→ 料金画面（`BillingClient.tsx` の行 + `hasVideoOption` + `VIDEO_OPTION_UI_NAMES`）→ ADM-008 の絞り込み（`users/page.tsx` の `VALID_OPTIONS` + `users/filters.tsx`）→ `.env.local.example` に `STRIPE_PRICE_*`。Webhook（`handleVideoOption`）・銀行振込の動画有効化（`activateBankTransferVideoOptionAction`）・メール（`sendVideoActivatedEmails`）は `isVideoOption()` / `VideoOptionType` 経由で自動的に対応する。`option_subscriptions.option_type` は CHECK 無しの text（P12 で `bank_transfer_requests` を廃止したため、DB 側の列挙は無くなった）
...
- 期限は管理しない（P12）。銀行振込行は運営が「無効にする」まで有効。`expire-options` cron は銀行振込の補償を対象外にしたまま（既存契約の保護）
- 詳細: `docs/requirements/p12-bank-transfer-onoff-implementation-notes.md`、`.kiro/steering/database-schema.md`「銀行振込の運用」
