...
- **買い切り動画系オプション（`video` / `video_workplace` / `video_shooting`）を足すときの触り方（P7 で統合済み）**: `src/lib/billing/options.ts` の `OptionType` / `OPTION_LABELS` / `OPTION_PRICES_TAX_INCLUDED` / `VIDEO_OPTION_TYPES` に追加 → Checkout（`billing/actions.ts` の Zod スキーマ・`priceIdForOption`・`buildSuccessUrl`）→ 銀行振込申込（`bank-transfer-actions.ts` の enum）→ 料金画面（`BillingClient.tsx` の行 + `hasVideoOption` + `VIDEO_OPTION_UI_NAMES`）→ `.env.local.example` に `STRIPE_PRICE_*`。Webhook（`handleVideoOption`）・ADM-026 有効化・メール（`sendVideoActivatedEmails`）は `isVideoOption()` / `VideoOptionType` 経由で自動的に対応する。`option_subscriptions.option_type` は CHECK 無しの text だが、**`bank_transfer_requests_target_consistency` CHECK がオプション種別を列挙している**ため、migration で許可リストに追加すること（P7 で `20260902150000` が前例。忘れると銀行振込の申込だけが「受付に失敗しました」で落ちる）
